	...
	while true do
		...
		if DoesEntityExist(closestObject) then
			...
			if GetDistanceBetweenCoords(pedCoords, sitCoords, true) <= 1.0 then
                RageUI.Text({ message = "Appuyez sur ~r~[E]~s~ pour vous asseoir sur la chaise roulante", time_display = 1 })
			...
			if GetDistanceBetweenCoords(pedCoords, pickupCoords, true) <= 1.0 then
                RageUI.Text({ message = "Appuyez sur ~r~[E]~s~ pour prendre la chaise roulante", time_display = 1 })
				if IsControlJustPressed(0, 38) then
