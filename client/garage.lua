        ...
        if ESX.PlayerData.job and ESX.PlayerData.job.name == 'ambulance' then
            ...
            if NordGarage <= Config.Marker.DrawInteract then
                ...
                if IsControlJustPressed(1,51) then
                    for _, v in pairs(Config.Garage.voiture) do
            ...
            if SudGarageHeli <= Config.Marker.DrawInteract then
                ...
                if IsControlJustPressed(1,51) then
                    for _, v in pairs(Config.Garage.helico) do
                        if v.category == nil then
                            ESX.TriggerServerCallback('eAmbulance:getVehGarage', function(amount)
            ...
            if SudGarageBato <= Config.Marker.DrawInteract then
                ...
                if IsControlJustPressed(1,51) then
                    for _, v in pairs(Config.Garage.bato) do
                        if v.category == nil then
                            ESX.TriggerServerCallback('eAmbulance:getVehGarage', function(amount)
...
function SpawnBato(car)
    local bato = GetHashKey(car)
    RequestModel(bato)
    while not HasModelLoaded(bato) do
        RequestModel(bato)
        Citizen.Wait(0)
    end
    local vehicle = CreateVehicle(bato, Config.SpawnGarage.Bato.Position, Config.SpawnGarage.Bato.Heading, true, false)
...
function SpawnHelico(car)
    local helico = GetHashKey(car)
    RequestModel(helico)
    while not HasModelLoaded(helico) do
        RequestModel(helico)
        Citizen.Wait(0)
    end
    local vehicle = CreateVehicle(helico, Config.SpawnGarage.Helico.Position, Config.SpawnGarage.Helico.Heading, true, false)
    SetEntityAsMissionEntity(vehicle, true, true)
    SetPedIntoVehicle(GetPlayerPed(-1),vehicle,-1)
    SetVehicleMaxMods(vehicle)
    TriggerServerEvent('ddx_vehiclelock:givekey', 'no', GetVehicleNumberPlateText(vehicle))
...
function SpawnNord(card)
    local spawnnord = GetHashKey(card)

    RequestModel(spawnnord)
    while not HasModelLoaded(spawnnord) do
        RequestModel(spawnnord)
        Citizen.Wait(0)
    end

    local vehicle = CreateVehicle(spawnnord, Config.SpawnGarage.VoitureNord.Position, Config.SpawnGarage.VoitureNord.Heading, true, false)
    SetEntityAsMissionEntity(vehicle, true, true)
    SetPedIntoVehicle(GetPlayerPed(-1),vehicle,-1)
    SetVehicleMaxMods(vehicle)
    TriggerServerEvent('ddx_vehiclelock:givekey', 'no', GetVehicleNumberPlateText(vehicle))
...
function SpawnSud(car)
    local car_sud = GetHashKey(car)

    RequestModel(car_sud)
    while not HasModelLoaded(car_sud) do
        RequestModel(car_sud)
        Citizen.Wait(0)
    end

    local vehicle = CreateVehicle(car_sud, Config.SpawnGarage.VoitureSud.Position, Config.SpawnGarage.VoitureSud.Heading, true, false)
    SetEntityAsMissionEntity(vehicle, true, true)
    SetPedIntoVehicle(GetPlayerPed(-1),vehicle,-1)
    SetVehicleMaxMods(vehicle)
    TriggerServerEvent('ddx_vehiclelock:givekey', 'no', GetVehicleNumberPlateText(vehicle))
...
function GarageAmbulanceNord()
    ...
    while AmbuNord do
        ...
        RageUI.IsVisible(AmbuNord, true, true, true, function()
            ...
            RageUI.ButtonWithStyle("Ranger le véhicule dans le stock", nil, Config.RightLab,true, function(_, _, Selected)
                if Selected then
